...
def is_version_greater_than(version_str, compare_to="3.2", can_equal=False):
    def parse_version(version):
        return tuple(map(int, (version.split("."))))

    version_tuple = parse_version(version_str)
    compare_to_tuple = parse_version(compare_to)
    return version_tuple >= compare_to_tuple if can_equal else version_tuple > compare_to_tuple
